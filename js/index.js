...
new Swiper(".swiper-container", {
    ...
    onSlideChangeEnd: function (swiper) {
        ...
        var arySlisers = swiper.slides;
        for (var i=0; i<arySlisers.length; i++){

            if (i === curIndex){
                if (curIndex==0 || curIndex==arySlisers.length-2){
                    arySlisers[i].id = 'page'+(arySlisers.length-2);
                } else if (curIndex==1 || curIndex==arySlisers.length-1){
                    arySlisers[i].id = 'page1';
                } else {

                    arySlisers[i].id = 'page'+curIndex;
                }
            } else {
                arySlisers[i].id = '';
            }
